...
export const payrollRoutes: FastifyPluginAsync<PayrollRouteOpts> = async (app, opts) => {
  ...
  app.post('/payroll/payouts', manage, async (req) => {
    const { clinicId, viewer } = viewerOf(req)
    const input = validateInput(payoutCreateSchema, req.body)
    return ok(await service.createPayout(opts.deps, clinicId, viewer.userId, input))
  })

  app.delete('/payroll/payouts/:id', manage, async (req) => {
    const { clinicId, viewer } = viewerOf(req)
    const { id } = req.params as { id: string }
    await service.removePayout(opts.deps, clinicId, viewer.userId, id)
